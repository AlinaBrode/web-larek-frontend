...
* данное приложение следует MVP (Model View Presenter) а не MVC (Model View Controller) подходу; что видно из следующего:
* код, который располагается в index.ts, исполняет роль Presenter
* этот код активно управляет View и обновляет Model
* Controller обычно пассивен и в основном обрабатывает приходящие запросы
* в этом приложении View и Model не взаимодействуют, как было бы в MVC подходе
* View не обновляется, отслеживая состояние Model, обновление идёт строго через index.ts т.е. Presenter
* бизнес логика сосредоточена в index.ts, что характерно для MVP 
* пользовательский ввод обрабатывается в index.ts, что является ещё одной чертой MVP
...
* SellItem - описание товара (sellitems.ts)
  ...
  * price number | null - цена (синапсов)
...
* SellItemsFromAPI - тип - как информация о товарах приходит с сервера (sellitems.ts) 
...
### Базовые классы
* BasePopup - шаблонированный базоый класс, шаблонный параметр - интерфейс с набором полей, которые должны отображаться в попапе
  ...
  * show - показать попап
* Component - базовый шаблонный класс для элемента отображения
  * шаблонный параметр задаёт набор свойств, отображаемых с помощью данного класса 
